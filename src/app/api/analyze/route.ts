...
export async function POST(request: NextRequest) {
  if (isProcessing) {
    return new Response(
      JSON.stringify({ error: "이미 분석이 진행 중입니다. 완료 후 다시 시도해주세요." }),
  ...
  try {
    ...
    if (file.size > maxSize) {
      isProcessing = false;
      return new Response(
        JSON.stringify({ error: "파일 크기가 10MB를 초과합니다." }),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    if (!allowedTypes.includes(file.type)) {
      isProcessing = false;
      return new Response(
        JSON.stringify({ error: `지원하지 않는 파일 형식입니다. (${file.type})` }),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
